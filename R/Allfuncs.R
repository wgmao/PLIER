            ...
            if (i == iter.full & !L3.given) {
                ...
                if (abs(frac - curfrac) > 1/k) {
                  ...
                  while (biter < 150 & (biter < 1 | abs(frac - curfrac) > 1/k | npos == 0)) {
                    ...
                    if (curfrac > frac) {
                      # increase penatly check if the limits have been reached
                      if ((L3_2 - L3_1) < 1e-07) {
                        L3_2 <- L3_2 * 100
                      }
...
#' @export
plotTopZallPath <- function(plierRes, data, priorMat, top = 10, index = NULL, regress = F, fdr.cutoff = 0.2, ...) {
    pval.cutoff <- max(plierRes$summary[plierRes$summary[, 5] < fdr.cutoff, 4])
    ii <- which(colSums(plierRes$U) > 0)
    if (!is.null(index)) {
        ii <- intersect(ii, index)
...
scadZ <- function(Z, ii = 1:ncol(Z), lambda) {
    ...
    # Zt[,ii]=apply(Zn$mat[,ii], 2, function(x){scad(x,lambda)}) Zt[,ii]=sweep(Zt[, ii],2, Zn$ss[ii], '*')
...
softZ <- function(Z, ii = 1:ncol(Z), lambda) {
    ...
    # Zt[,ii]=apply(Zn$mat[,ii], 2, function(x){quicksoft(x,lambda)}) Zt[,ii]=sweep(Zt[, ii],2, Zn$ss[ii], '*')
    Zt[, ii] <- apply(Z[, ii], 2, function(x) {
        quicksoft(x, lambda)
    })
    Zt
...
PLIERsparse <- function(data, priorMat, svdres = NULL, k = NULL, L1 = NULL, L2 = NULL, L3 = NULL, frac = 0.7, max.iter = 350, trace = F, scale = T, Chat = NULL, maxPath = 10, doCrossval = T, 
    ...
    solveU <- function(Z, Ur, C, L3, penalty.factor, glm_alpha) {
        ...
        for (j in 1:ncol(U)) {
            ...
            # if(conditional){ iigenes=which(Z[,j]>0) } else{
            iigenes <- 1:nrow(Z)
            # }
            Zr <- rank(-Z[iigenes])
    ...
    for (i in 1:max.iter) {
        if (i >= iter.full.start) {
            ...
            # find the active pathways ii=which(apply(Us,1,min)<=maxPath)
