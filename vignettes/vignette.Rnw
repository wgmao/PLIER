...
@
Run PLIER with all default parameters. We set doCrossval=F to turn off gene hold-out cross-validation. Since we want to check correspondance between estimates and measured proportions it is best to use all the of the available pathway information.
<<echo=TRUE>>=
plierResult=PLIER(dataWholeBlood, allPaths, doCrossval=F)
@
...
Load data. Here we also include the Cibersort LM22 signature are markers (svmMarkers).
<<echo=TRUE>>=
data(bloodCellMarkersIRISDMAP)
data(svmMarkers)
data(canonicalPathways)
...
<<echo=TRUE, fig.height=12, tidy=T>>=
cm=commonRows(usoskin, usoskinPath)
